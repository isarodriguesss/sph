    ...
    def _rastro_segmento(self, fluid, lider, agar, tree):
        ...
        for k in lider:
            ...
            if RASTRO_BAIA > 0.0 and len(sel):
                sel = self._guarda_baia(fluid, k, p1, agar, sel, w)
            for j in sel:
                mae.setdefault(int(agar[j]), int(k))
        return mae

    def _guarda_baia(self, fluid, k, p1, agar, sel, w):
        ...
        viz = ctree.query_ball_point(q, g)
        todos = np.unique(np.concatenate([np.asarray(v, dtype=int) for v in viz] + [np.zeros(0, int)]))
        if len(todos) == 0:
            return sel
        c = np.column_stack([fluid.x[col[todos]], fluid.y[col[todos]]])
        ...
        vizinho = set(todos[d_poly > w + 0.5 * dx].tolist())
        ok = np.array([not any(int(j) in vizinho for j in v) for v in viz], dtype=bool)
        return sel[ok]
