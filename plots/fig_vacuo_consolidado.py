...
nbr = np.array(
    [len(tree.query_ball_point([x[i], y[i]], 2.0 * h)) - 1 for i in idx_holes]
)
...
ax[1].set_title(
    f"2) As 'holes' TÊM vizinhos ({int(holes.sum())} partículas)\n"
    "0 isoladas → não é espaço vazio",
    fontsize=13,
)
