...
def cubic_spline_w(r, h):
    # W do kernel do solver em numpy (logica do wake): CubicSpline ou WendlandQuintic 2D
    q = np.asarray(r) / h
...
# Kernel e comprimento de suavizacao (licao #88). O spline cubico sofre instabilidade de
# PAREAMENTO quando a compressao dobra a vizinhanca (64-121 vizinhas em 2h contra 37): e o
# que colapsou o dt do P2 apos t~75. O Wendland C2 e estavel ao pareamento para qualquer
# numero de vizinhas (Dehnen & Aly 2012, MNRAS 425:1068; Liu §6.4). H_FACTOR=1.92 iguala o
# desvio-padrao do kernel ao do cubico em 1.8 (1.43 dx), e medido no P2 em t=50 isso muda
# sigma_a e |grad cs| na frente em menos de 3%. "cubic"/1.8 reproduz o P2 bit-a-bit.
KERNEL = "cubic"
H_FACTOR = 1.8
    ...
    def post_step(self, solver):
        ...
        if use_floor and solver.count % FLOOR_FREQ == 0:
            ...
            if int(np.count_nonzero(_corpo)) > 20:
                _dx = FLOOR_CELL * float(fl.h[0]) / H_FACTOR
                _rc = 1.2 * float(
