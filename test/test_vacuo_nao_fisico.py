...
def main():
    ...
    print(f"Snapshot: {hdf5.split('/')[-1]}   N={len(x)}   colônia={int((rho_b>0.05).sum())}\n")
    ...
    nbr_counts = np.array([len(tree.query_ball_point([x[i], y[i]], 2.0 * h)) - 1
                           for i in holes]) if len(holes) else np.array([0])
    n_isolado = int((nbr_counts < 5).sum())  # < 5 vizinhos = quase-vácuo real
    frac_hole = len(holes) / len(colony)
    print(f"(1) O 'buraco' é queda de densidade, não espaço vazio:")
    print(f"    'holes' no scatter (rho/rho0<{HOLE_THRESH}): {len(holes)} "
          f"({frac_hole:.0%} da colônia)")
    print(f"    vizinhos por 'hole' dentro de 2h: mediana={int(np.median(nbr_counts))}, "
          f"mín={int(nbr_counts.min())}")
    print(f"    partículas quase-isoladas (<5 vizinhos): {n_isolado}  "
    ...
    ok = a1 and a2 and a3
    print("-" * 62)
    print("✅ O 'VÁCUO' É ARTEFATO DE RENDERIZAÇÃO, NÃO PROBLEMA FÍSICO"
          if ok else "❌ revisar")
    print("=" * 62)
