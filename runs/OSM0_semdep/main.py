...
# OSM1 — INFLUXO OSMOTICO como mecanismo de construcao, no lugar da deposicao ([T2]
# Srinivasan: a fronteira avanca por influxo de fluido do agar, V0 ~ osmolitos). Agar
# vizinho de biomassa VIVA e absorvido a taxa OSM_K * sum_j V_j rho_b_j W_ij, massa e
# particulas conservadas. Absorvido = QUIMICA de agar (rho_b continua 0: nao cresce, nao
# produz cs, nao consome c_n, e colonizavel) + MECANICA de colonia (a BiomassEOS le
# max(rho_b, phi_osm), com phi_osm = 0.4 -> fade 0.84). Nao e semente (so VIVAS impulsionam),
# entao nao vira flood-fill. phi_osm = 0 em toda parte reproduz o E11 bit-a-bit.
use_osm = False
OSM_K = 32.0
OSM_PHI = 0.4
OSM_FREQ = 20
...
class SwarmApp(Application):
    def initialize(self):
        ...
        self._osm_t = 0.0
        self._osm_total = 0
        self._osm_rng = np.random.RandomState(SEED + 1 if SEED is not None else None)
    ...
    def post_step(self, solver):
        ...
        if use_osm and solver.count % OSM_FREQ == 0:
            fl = self.particles[0]
            _dt = solver.t - self._osm_t
            self._osm_t = solver.t
            _viva = np.where((fl.is_filler < 0.5) & (fl.rho_b_grown > 0.1))[0]
            if _dt > 0.0 and len(_viva) > 0:
                _h0 = float(fl.h[0])
                _P = np.column_stack([fl.x, fl.y])
                _viz = cKDTree(_P).query_ball_point(_P[_viva], 2.0 * _h0)
                _nv = np.array([len(v) for v in _viz])
                _jj = np.concatenate([np.asarray(v, dtype=int) for v in _viz])
                _ii = np.repeat(_viva, _nv)
                _rij = np.hypot(fl.x[_jj] - fl.x[_ii], fl.y[_jj] - fl.y[_ii])
                _w = (fl.m[_ii] / fl.rho[_ii]) * fl.rho_b_grown[_ii] * cubic_spline_w(
                    _rij, _h0
                )
                _drive = np.bincount(_jj, weights=_w, minlength=len(fl.x))
                _cand = (
                    (fl.phi_osm < 0.5 * OSM_PHI)
                    & (fl.is_filler < 0.5)
                    & (fl.rho_b_grown < 0.1)
                    & (_drive > 0.0)
                )
                _prob = 1.0 - np.exp(-OSM_K * _drive * _dt)
                _conv = _cand & (self._osm_rng.random_sample(len(fl.x)) < _prob)
                if np.any(_conv):
                    fl.phi_osm[_conv] = OSM_PHI
                    self._osm_total += int(_conv.sum())
            if solver.count % print_freq == 0:
                print(
                    f"osmotico t={solver.t:.1f}s: absorvidas total={self._osm_total} "
                    f"(phi_osm>0: {int(np.count_nonzero(fl.phi_osm > 0))}, vivas={len(_viva)})"
                )
