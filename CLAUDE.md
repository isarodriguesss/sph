...
**Reprodutibilidade (obrigatoria a partir de 2026-08-06):** `SEED` fixa em [main.py](main.py), propagada a `create_initial_state` ([src/particles.py](src/particles.py)). Antes disso, `noise = 0.10*np.random.randn(...)` era **nao-semeado** — com `n_bio ≈ 70-90`, o erro binomial de `frac_lowsig_bio` e **±8.6%**, e diferenças finas entre rotas nao eram atribuiveis. Comparacoes entre rotas devem ser puramente mecanicas.
