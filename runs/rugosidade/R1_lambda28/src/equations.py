...
class SurfactantEquation(Equation):
    def __init__(
        ...
        filler_conduz=0.0,
        filler_D=0.0,
        filler_D_interno=0.0,
        filler_lambda_ratio=0.5,
        agar_lambda_ratio=0.5,
    ):
        self.hill_k2 = hill_k * hill_k
        self.filler_conduz = filler_conduz
        self.filler_D = filler_D
        self.filler_D_interno = filler_D_interno
        self.filler_lambda_ratio = filler_lambda_ratio
        self.agar_lambda_ratio = agar_lambda_ratio
    ...
    def post_loop(self, d_idx, d_rho_b_grown, d_a_c_s, d_cs, d_noise, d_c_n, d_is_filler):
        if d_is_filler[d_idx] > 0.5:
            if self.filler_conduz > 0.5:
                sink_f = self.lambda_ * self.filler_lambda_ratio * d_cs[d_idx]
            ...
            if rho_b < 0.1:
                lambda_eff = self.lambda_ * self.agar_lambda_ratio
            else:
...
class ForcaContornoPilar(Equation):
    # pass-l-aprovado: Pass L desbloqueado em 2026-09-17 (CLAUDE.md §12). Forca de contorno por
    # particulas de Monaghan & Kajtar (2009), a = K * (x_ij/r^2) * W(r, h_w) * 2 m_j/(m_i+m_j),
    # com h_w = 0.5 dx (alcance 1 dx) em vez do h da simulacao: com 2h = 3.6 dx a forca empurraria
    # todo o fluido a ate 3.6 dx do pilar (v induzida 0.2 a 1 dx) e abriria um anel de vazio, que
    # no agar nao cicatriza (licao #40). Calibracao em docs/PLANO_RUGOSIDADE.md secao 4.
    def __init__(self, dest, sources, K=0.005, hw=0.0269):
        self.K = K
        self.hw = hw
    ...
    def loop(self, d_idx, s_idx, d_au, d_av, d_m, s_m, XIJ, RIJ, d_ax_rep, d_ay_rep):
        if RIJ > 1e-12:
            ...
            w = 0.0
            if q < 1.0:
                w = self.sig * (1.0 - 1.5 * q * q + 0.75 * q * q * q)
            elif q < 2.0:
                w = self.sig * 0.25 * (2.0 - q) * (2.0 - q) * (2.0 - q)
            if w > 0.0:
                mfac = 2.0 * s_m[s_idx] / (d_m[d_idx] + s_m[s_idx])
